...
vec2 f(vec3 p) {
	...
	float box_size = .8;
	float box1 = box(rot_box1 * p - vec3(-box_size * 1.5, -3. + box_size * 2, -box_size), vec3(box_size, box_size * 2., box_size));
	float box2 = box(rot_box2 * p - vec3( box_size *1.5, -3. + box_size,  box_size), vec3(box_size));
