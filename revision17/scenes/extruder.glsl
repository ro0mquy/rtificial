...
float fScene(vec3 p) {
	...
	vec3 loco_index = pMirrorLoco(p_torus.xy, vec2(ext_obj_loco_rt_float));
...
Material getMaterial(MaterialId materialId) {
	...
	if (materialId.id == mat_id_bounding) {
		...
		float px_before = (materialId.misc.w/ext_extrude_h_rt_float/ext_extrude_freq_rt_float + 1.) / 2.; // {0,1}
