	...
	defaultShader(context)
{
	...
	StrahlenwerkApplication::getInstance()->getProject().registerPostprocListener(this);
	reloadPostproc();
}
...
void Renderer::reloadPostproc() {
	auto shaders = StrahlenwerkApplication::getInstance()->getProject().loadPostprocShaders(context);
	if(!shaders.empty()) {
